"""LightGBM model wrapper for AutoML Arena."""
...
class LightGBMModel(BaseAutoMLModel):
    """LightGBM wrapper supporting hyperparameter search."""
    ...
    def prepare_estimator_params(self, params: Dict[str, Any], y: Optional[Any] = None) -> Dict[str, Any]:
        """Inject scale_pos_weight for imbalanced binary classification."""
        effective_params = dict(params)
        if self.task_type != "classification" or y is None:
            return effective_params
        if "scale_pos_weight" in effective_params or "class_weight" in effective_params:
            return effective_params
    ...
    def build_estimator(self, params: Dict[str, Any]) -> BaseEstimator:
        """Instantiate LGBMClassifier or LGBMRegressor."""
        estimator_params = dict(params)
        estimator_params.pop("class_imbalance_ratio", None)
        if self.task_type == "classification":
            return LGBMClassifier(random_state=42, verbose=-1, **estimator_params)
        else:
            return LGBMRegressor(random_state=42, verbose=-1, **estimator_params)
